...
export default function Navbar() {
  ...
  // Close the mobile menu whenever the route changes or we leave mobile.
  useEffect(() => { setMenuOpen(false); }, [pathname]);
  useEffect(() => { if (!isMobile) setMenuOpen(false); }, [isMobile]);

  const solidBg = scrolled || (isMobile && menuOpen);
  ...
  return (
    <nav
      style={{
        ...
        alignItems: "center",
        background: solidBg ? "rgba(232, 224, 213, 0.97)" : "transparent",
        backdropFilter: solidBg ? "blur(8px)" : "none",
    ...
    >
      ...
      {/* Right: social icons (desktop) OR burger (mobile) */}
      {!isMobile ? (
        ...
        >
          <span style={{
            display: "block", width: "24px", height: "2px", background: "#6B1F1F",
            borderRadius: "2px", transition: "transform 0.3s ease, opacity 0.3s ease",
      ...
      {isMobile && (
        ...
        >
          ...
          <div style={{ display: "flex", gap: "24px", alignItems: "center", padding: "20px clamp(20px, 6vw, 80px)" }}>
            <a
              href="https://www.linkedin.com/in/joffray-dealberto/"
              target="_blank"
              rel="noopener noreferrer"
